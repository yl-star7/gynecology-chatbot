...
export interface AuthPort {
  signInWithPhonePassword(input: { phoneNumber: string; password: string }): Promise<AuthenticatedUser>;
  verifyPhone(input: { phoneNumber: string; verificationCode: string }): Promise<{ verificationToken: string }>;
  setPassword(input: { verificationToken: string; password: string }): Promise<AuthenticatedUser>;
...
export interface CalendarPort {
  getHomeCalendar(input: { month: string }): Promise<HomeViewData["calendarDays"]>;
}
...
export interface AdminUserPort {
  ...
  updatePhoneNumber(input: { userId: string; phoneNumber: string; reason: string }): Promise<void>;
