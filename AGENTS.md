...
- UI 문구는 운영자와 앱 사용자가 실제로 이해하는 말부터 씁니다. 내부 키, DB 컬럼, YAML 경로, `fallback`, `mood_intake` 같은 구현 용어를 기본 화면에 노출하지 않습니다.
- 흐름형 화면은 “어디로 가라”가 아니라 “여기서 보고 고친다”가 기본입니다. 각 흐름 조각을 누르면 현재 문구, 프롬프트, 참조를 같은 화면의 모달이나 패널에서 보고 수정·추가할 수 있어야 합니다.
- 기술 식별자가 꼭 필요하면 디버그/상세 영역에 보조 정보로만 둡니다. 기본 라벨과 제목은 앱에 보이는 말과 운영자가 쓰는 업무 언어로 작성합니다.
- 관리자 화면이라도 설명문을 길게 쓰지 않습니다. 상태, 선택값, 편집 가능한 항목을 짧고 직접적인 라벨로 보여줍니다.
...
| `/api/admin/rag-provider` | GET/PUT | RAG 백엔드 설정 (schift) |
...
- DB 접근은 Prisma/Cloud SQL 기반 repository 또는 기존 `@gynecology-chatbot/db` 패턴을 우선 사용
- API 라우트에서 ad-hoc SQL을 늘리지 말고, 기존 admin/mobile repository와 service 계층을 재사용
- content 스키마 테이블은 기존 repository 경계를 통해 접근
