...
- **근거**:
  - `supabase/migrations/20260331172420_move_content_to_public_and_drop_allowlist.sql:27` — `CHECK (week_number BETWEEN 1 AND 40)`
  - `supabase/migrations/drizzle/0000_wet_mattie_franklin.sql:162` — `CHECK ("pregnancy_weeks"."week_number" BETWEEN 1 AND 40)`
  - `supabase/migrations/20260417143000_add_content_paraphrase_tables.sql:72-73` — `CHECK (source_week_number BETWEEN 1 AND 40)`
...
`content.content_pregnancy_documents.category` / `content.content_knowledge_items.category` 는 `varchar(100) NOT NULL` 자유 입력입니다(`supabase/migrations/drizzle/0000_wet_mattie_franklin.sql:99`, `apps/web/prisma/schema.prisma:239,259`). 그러나 실제 코드에서 사용되는 값은 encyclopedia paraphrase 파이프라인의 `ALLOWED_SECTION_CATEGORIES` 뿐입니다.
...
2. **시드 SQL**: `supabase/migrations/` 에 `YYYYMMDD_add_tag_<namespace>_<value>.sql` 마이그레이션으로 `content.tags` INSERT.
3. **코드 반영** (네임스페이스별):
...
```sql
-- supabase/migrations/YYYYMMDD_seed_tag_namespaces.sql
...
| week 범위 제약 | `supabase/migrations/20260331172420_move_content_to_public_and_drop_allowlist.sql` | 27 |
| week 범위 제약 (drizzle) | `supabase/migrations/drizzle/0000_wet_mattie_franklin.sql` | 162 |
| paraphrase week 제약 | `supabase/migrations/20260417143000_add_content_paraphrase_tables.sql` | 72-73 |
| pregnancy_documents.category 스키마 | `supabase/migrations/drizzle/0000_wet_mattie_franklin.sql` | 99, 223 |
