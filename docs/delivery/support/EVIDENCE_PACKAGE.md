...
- 검수자가 **문서를 읽지 않고도 폴더만 보고 항목을 추적**할 수 있게 AC/LIM ID를 최상위 기준으로 사용합니다.
- 하나의 판정을 위해 여러 증빙이 필요한 경우, `request → response → screenshot → memo` 순서로 파일을 배치합니다.
- 원본과 제출본이 다를 경우 `raw/`와 `redacted/`를 분리하거나 파일명에 `-redacted`를 붙입니다.
- PDF, PNG, CSV처럼 서로 다른 형식이 섞여도 동일 항목 안에서는 파일명 규칙을 통일합니다.
- 압축 전 `manifest.csv` 또는 `README.md`에서 각 파일의 목적을 한 줄씩 설명합니다.
...
| 파일 | 역할 |
|---|---|
| `README.md` | 패키지 전체 설명, 검수 범위, 압축일, 제출자 기재 |
| `manifest.csv` | 파일명 / 항목 ID / 설명 / 생성일시 / 비고 관리 |
| `acceptance-summary.csv` | AC/LIM별 판정 요약 및 증빙 위치 매핑 |

### 5-2. 민감정보 마스킹 기준

| 대상 | 처리 방법 |
|---|---|
| 전화번호 | 가운데 자리 마스킹 (`010-****-1234`) |
| 액세스 토큰/세션 토큰 | 전체 마스킹 또는 앞 4자리만 표시 |
| API 키/비밀값 | 전부 `***` 처리 |
| 개인정보 포함 화면 | 이름/전화번호/예정일 등 식별 정보 비식별 처리 |
| DB 조회 결과 | 검수와 무관한 열은 제거 후 제출 |

### 5-3. 검수자 관점에서 빠지기 쉬운 누락 항목

- 요청 본문은 있는데 응답 본문이 없는 경우
- 화면 캡처는 있지만 어떤 AC에 대응하는지 파일명에 표시되지 않은 경우
- 동일 `sessionId` 연결 증빙이 없어 AC-002와 AC-006이 분리되어 보이는 경우
- 운영 증빙에 실행 일시/실행자가 없어서 실제 점검 여부를 판단할 수 없는 경우
- 마스킹본만 있고 원본 기준 설명이 없어 진위 판단이 어려운 경우
