...
| 항목 | 확인 내용 |
|---|---|
| API 기준 URL | 모바일 API와 관리자 웹의 기준 URL을 사전에 확정합니다. (운영/스테이징 구분 포함) |
| 계정 상태 | 검수용 사용자/관리자 계정이 실제 로그인 가능한 상태인지 사전 확인합니다. |
| OTP 수신 | 검수용 전화번호가 실제 OTP 수신 가능한 번호인지 확인합니다. |
| 브라우저 상태 | 관리자 검수는 시크릿 창 또는 세션 초기화 상태에서 시작합니다. |
| 로그 저장 | 요청/응답 JSON, 캡처 PNG, 메모 MD를 AC별 폴더에 즉시 저장합니다. |
| 점검 시각 | 모든 증빙에 실행 일시(예: `2026-04-08 15:30 KST`)를 기록합니다. |
...
- 모든 AC는 **실행 순서, 실행자, 환경(운영/스테이징), 결과**를 함께 기록합니다.
- API 응답은 가능하면 원문 JSON 그대로 저장하고, 캡처본만 별도로 제출합니다.
- `Pass` 판정은 단순 200 응답이 아니라 **계약상 기대 결과가 실제로 충족되는지**까지 확인했을 때만 부여합니다.
- 동일 검수 세션에서 생성된 `sessionId`, 요청 시각, 화면 캡처 시각이 연결되도록 파일명을 맞춥니다.
- 실패한 항목도 삭제하지 않고 `fail-*` 또는 `retry-*` 접두사로 남겨 검수 이력을 보존합니다.
...
```md
- 실행 항목: AC-00X
- 실행 일시: 2026-04-08 15:30 KST
- 실행자: 홍길동
- 대상 환경: 운영 / 스테이징
- 경로 또는 엔드포인트: /admin/operations, POST /api/mobile/chat
- 사용 식별자: sessionId=...
- 결과: Pass / Fail / 재시도
- 특이사항: OTP 지연, 응답 지연, 운영 데이터 차이 등
```

### 5.4 실패 또는 보완 필요 시 기록 방법

- 동일 항목을 재시도했다면 최초 실패 로그와 재시도 로그를 모두 남깁니다.
- 운영 환경 차이로 실패한 경우, 코드 결함인지 환경 설정 문제인지 구분해서 메모합니다.
- 화면 캡처만으로 설명이 부족한 경우 `memo.md`에 클릭 순서와 화면 문구를 텍스트로 적습니다.
- 판정 보류 항목은 `ACCEPTANCE_CHECKLIST.md` 비고란과 동일 표현으로 맞춥니다.
