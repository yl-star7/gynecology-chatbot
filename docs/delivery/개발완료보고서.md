...
임산부(예비맘) 대상 AI 기반 모성간호 상담 챗봇 서비스를 구축하였습니다. 사용자 모바일 앱(iOS/Android)과 관리자 웹 콘솔, legacyBackend 백엔드로 구성됩니다.
...
| 7 | 서버 비용 (6개월) | 900,000원 | 운영 중 | legacyBackend + legacyBackend 배포 완료, 9월까지 운영 |
| 8 | 웹사이트 도메인 (1년) | 50,000원 | 완료 | legacyBackend 도메인 적용 |
...
```
사용자 (모바일 앱) ──→ API 서버 (Next.js / legacyBackend)
                            ├── legacyBackend (PostgreSQL + Storage)
                            ├── Google Gemini (AI 응답)
                            ├── Schift SDK (RAG + 워크플로우)
                            ├── Twilio (SMS/OTP)
                            └── Expo Server SDK (푸시 알림)

관리자 (웹 콘솔) ────→ Admin API (Next.js / legacyBackend)
```
...
| 데이터베이스 | PostgreSQL 15 (legacyBackend Cloud) |
| AI 엔진 | Google Gemini + Schift SDK |
| 인증/SMS | Twilio Verify + Messages |
| 푸시 알림 | Expo Server SDK (FCM/APNs) |
| 파일 저장소 | legacyBackend Storage |
| 배포 (웹) | legacyBackend (도쿄 리전) |
...
| DB 마이그레이션 | `legacyBackend/migrations/` | 원격 legacyBackend history와 맞는 active chain 3건 |
| Legacy 마이그레이션 | `legacyBackend/migrations_legacy/` | baseline 이전 historical SQL 보관 |
...
![모바일 로그인](screenshots/08_mobile_login.jpg)

### 10.2 관리자 콘솔 — 운영 상태

알림 발송, 알림 스케줄 설정, 브랜딩(마스코트/설문) 관리 화면입니다.

![운영 상태](screenshots/01_operations.jpg)

### 10.3 관리자 콘솔 — 계정 관리

사용자 검색, 전화번호 변경, 세션 초기화, 사용 중단/재개 기능입니다.

![계정 관리](screenshots/02_accounts.jpg)

### 10.4 관리자 콘솔 — 참조 파일 (RAG 문서)

PDF/DOCX 파일 업로드 및 지식 문서 관리 화면입니다.

![참조 파일](screenshots/03_documents.jpg)

### 10.5 관리자 콘솔 — 지식 콘텐츠

주차별 고정 안내문 관리 화면입니다.

![지식 콘텐츠](screenshots/04_static.jpg)

### 10.6 관리자 콘솔 — 주차별 아기는요?

임신 5~40주차 콘텐츠 관리 (아기/엄마 정보, 체크리스트, 질문, 이미지)입니다.

![주차별 아기는요](screenshots/05_weeks.jpg)

### 10.7 관리자 콘솔 — 응답 워크플로우

AI 응답 워크플로우 설정 및 테스트 실행 화면입니다.

![응답 워크플로우](screenshots/06_policies.jpg)

### 10.8 관리자 콘솔 — 모니터링

실시간 사용자 이벤트 및 채팅 세션 감사 화면입니다.

![모니터링](screenshots/07_monitoring.jpg)
