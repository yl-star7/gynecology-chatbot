...
describe("/api/admin/approval-policy proxy", () => {
  ...
  beforeEach(() => {
    ...
    global.fetch = jest.fn(
      async () =>
        new Response(JSON.stringify({ requireApproval: true }), {
          status: 200,
  ...
  test("forwards GET requests to the API server", async () => {
    ...
    await expect(response.json()).resolves.toEqual({ requireApproval: true });
    expect(global.fetch).toHaveBeenCalledWith(
      "http://api.example.test/api/admin/approval-policy",
  ...
  test("forwards PUT requests to the API server", async () => {
    const response = await PUT(
      new Request("http://localhost:3000/api/admin/approval-policy", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
