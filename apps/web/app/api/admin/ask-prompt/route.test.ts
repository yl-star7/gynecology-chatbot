...
describe("/api/admin/ask-prompt proxy", () => {
  ...
  beforeEach(() => {
    ...
    global.fetch = jest.fn(async () =>
      new Response(JSON.stringify({ tonePrompt: "따뜻하게 답변해요." }), {
        status: 200,
  ...
  test("forwards GET requests to the API server", async () => {
    ...
    await expect(response.json()).resolves.toEqual({
      tonePrompt: "따뜻하게 답변해요.",
    });
    expect(global.fetch).toHaveBeenCalledWith(
      "http://api.example.test/api/admin/ask-prompt",
  ...
  test("forwards PUT requests to the API server", async () => {
    const response = await PUT(
      new Request("http://localhost:3000/api/admin/ask-prompt", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tonePrompt: "짧게 답변해요." }),
      }) as never,
    );
