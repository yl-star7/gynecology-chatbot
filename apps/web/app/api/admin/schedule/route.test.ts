...
describe("/api/admin/schedule proxy", () => {
  ...
  beforeEach(() => {
    ...
    global.fetch = jest.fn(async () =>
      new Response(JSON.stringify({ dailyCheckTime: "09:00" }), {
        status: 200,
  ...
  test("forwards GET requests to the API server", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "http://api.example.test/api/admin/schedule",
  ...
  test("forwards PUT requests to the API server", async () => {
    const response = await PUT(
      new Request("http://localhost:3000/api/admin/schedule", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dailyCheckTime: "08:30" }),
      }) as never,
    );
