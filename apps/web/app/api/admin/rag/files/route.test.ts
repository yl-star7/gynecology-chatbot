...
describe("/api/admin/rag/files proxy", () => {
  ...
  test("forwards GET requests to the API server", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      "http://api.example.test/api/admin/rag/files",
      expect.objectContaining({ method: "GET" }),
    );
  });

  test("forwards POST uploads to the API server", async () => {
    const response = await POST(
      new Request("http://localhost:3000/api/admin/rag/files", {
        method: "POST",
