      ...
      <aside className="app-sidebar">
        <div className="brand-block">
          ...
          <p className="brand-copy">
            운영 대시보드, RAG 데이터 설정, 워크플로우 설정, 유저 히스토리
            조회를 한 화면에 정리했습니다.
          </p>
        </div>
        ...
        <div className="sidebar-card">
          ...
          <p>
            사용자 검색, 계정 복구, RAG 데이터 운영, AI 워크플로우 점검, 세션
            히스토리 조회
          </p>
        </div>
      ...
      <section className="app-main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="ghost-chip" type="button" onClick={handleLogout}>
              로그아웃
            </button>
          </div>
        </header>
            ...
            <div className="table-list">
              {dashboard.recoveryActions.map((action) => (
                <div key={action.id} className="table-row">
                  ...
                  <div>
                    <span
                      className={`badge ${action.status === "completed" ? "badge-success" : "badge-warning"}`}
                    >
                      {action.status}
                    </span>
            ...
            <div className="admin-form">
              <label className="field-label">
                ...
                <select
                  ...
                  onChange={(event) => syncSelectedUser(event.target.value)}
              ...
              {actionMessage ? (
                <p className="inline-feedback">{actionMessage}</p>
              ) : null}
            </div>
          </section>
        </section>
        ...
        <section className="panel">
          <div className="panel-header">
            <div>
              <p className="eyebrow">User Actions</p>
              <h2>최근 사용자 액션</h2>
            </div>
            <span className="badge badge-neutral">Live Feed</span>
          </div>
          <div className="table-list">
            {dashboard.userActions.slice(0, 12).map((action) => (
              <div key={action.id} className="table-row">
                <div>
                  <strong>{action.userName}</strong>
                  <span>
                ...
                <div>
                  <span className="badge badge-neutral">
                    {action.sessionTitle ?? "앱 액션"}
                  </span>
                  <span>{action.occurredAtLabel}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="panel">
          ...
          <div className="profile-card profile-card-wide">
            ...
            <div className="profile-stack">
              ...
              <strong>
                {
                  ragDocuments.filter((document) => document.status === "ready")
                    .length
                }
              </strong>
            </div>
            <div className="profile-stack">
              ...
              <strong>
                {
                  ragDocuments.filter((document) => document.status === "draft")
                    .length
                }
              </strong>
            </div>
          ...
          <div className="admin-form">
            ...
            <div className="action-row">
              <label className="field-label field-flex">
                ...
                <input
                  className="field-input"
                  value={ragCategory}
                  onChange={(event) => setRagCategory(event.target.value)}
                />
              </label>
              <label className="field-label field-flex">
                <span>주차</span>
                <input
                  ...
                  inputMode="numeric"
                  value={ragWeek}
                  onChange={(event) => setRagWeek(event.target.value)}
                />
              </label>
            </div>
            <label className="field-label">
            ...
            <div className="action-row">
              <button
                className="primary-button"
                type="button"
                onClick={handleUploadRagDocument}
                disabled={isSubmitting}
              >
                RAG 문서 업로드
              </button>
          ...
          <div className="table-list">
            {ragDocuments.map((document) => (
              <div key={document.id} className="table-row">
                ...
                <div>
                  <span
                    className={`badge ${document.status === "ready" ? "badge-success" : "badge-warning"}`}
                  >
                    {document.status}
                  </span>
                  <span>
                    {document.chunkCount} chunks · {document.updatedAt}
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="table-list">
            {dashboard.workflowRules.map((rule) => (
              <div key={rule.id} className="table-row">
                ...
                <div>
                  <span
                    className={`badge ${rule.status === "active" ? "badge-success" : "badge-warning"}`}
                  >
                    {rule.status}
                  </span>
                  <span>
                ...
                <section className="panel inset-panel">
                  <div className="panel-header">
                    <div>
                      ...
                      <h2>선택 사용자 액션 타임라인</h2>
                    </div>
                    <span className="badge badge-neutral">Actions</span>
                  </div>
                  <div className="table-list compact-list">
                      ...
                      <div className="table-list compact-list">
                        {session.messages.map((message) => (
                          <div key={message.id} className="table-row">
                            ...
                            <div>
                              <span
                                className={`badge ${message.role === "assistant" ? "badge-success" : "badge-neutral"}`}
                              >
                                {message.role}
                              </span>
