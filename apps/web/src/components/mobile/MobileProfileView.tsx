...
import { fetchMobileProfile, appendUserIdToPath, resolveMobileUserId, updateMobileProfile } from "@/lib/mobile/web-mobile-api";
...
function resolveToneLabel(value?: string | null) {
  return TONE_OPTIONS.find((option) => option.value === value)?.label ?? value ?? "미설정";
}
      ...
      <div className="grid gap-4">
        ...
        <section className="rounded-[28px] border border-[var(--line)] bg-white/85 p-5 shadow-[var(--shadow)]">
          ...
          <form className="mt-4 grid gap-3" onSubmit={handleSave}>
            ...
            <label className="grid gap-2">
              <span className="text-sm font-medium text-[var(--text)]">태명</span>
              <input
                value={babyNickname}
            ...
            <label className="grid gap-2">
              <span className="text-sm font-medium text-[var(--text)]">예정 출산일</span>
              <input
                type="date"
                value={dueDate}
            ...
            <label className="grid gap-2">
              <span className="text-sm font-medium text-[var(--text)]">주 진료 병원</span>
              <input
                value={hospitalName}
            ...
            <label className="grid gap-2">
              <span className="text-sm font-medium text-[var(--text)]">매일 알림 시간</span>
              <input
                type="time"
            ...
            <label className="grid gap-2">
              <span className="text-sm font-medium text-[var(--text)]">상담 톤</span>
              <select
        ...
        <section className="rounded-[28px] border border-[var(--line)] bg-white/85 p-5 shadow-[var(--shadow)]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-soft)]">Pregnancy</p>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile?.pregnancyWeekLabel ?? "정보 없음"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4">
              <p className="text-sm text-[var(--text-soft)]">임신 일차</p>
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile ? `${profile.pregnancyDayCount}일` : "정보 없음"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4">
              <p className="text-sm text-[var(--text-soft)]">온보딩</p>
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile?.hasCompletedOnboarding ? "완료" : "미완료"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4 sm:col-span-3">
              ...
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile?.dueDate ?? "미설정"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4">
              <p className="text-sm text-[var(--text-soft)]">태명</p>
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile?.babyNickname ?? "미설정"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4">
              <p className="text-sm text-[var(--text-soft)]">주 진료 병원</p>
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile?.hospitalName ?? "미설정"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4">
              <p className="text-sm text-[var(--text-soft)]">매일 알림 시간</p>
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{profile?.notificationTime ?? "08:30"}</p>
            </div>
            <div className="rounded-[22px] border border-[var(--line)] bg-[rgba(20,34,20,0.03)] p-4 sm:col-span-3">
              <p className="text-sm text-[var(--text-soft)]">상담 톤</p>
              <p className="mt-2 text-lg font-semibold text-[var(--text)]">{resolveToneLabel(profile?.tonePreference)}</p>
            </div>
          </div>
        </section>

        <section className="rounded-[28px] border border-[var(--line)] bg-white/85 p-5 shadow-[var(--shadow)]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-soft)]">Actions</p>
          <div className="mt-4 flex flex-wrap gap-3">
            <Link href={appendUserIdToPath("/", resolvedUserId)} className="rounded-full bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-white">
              홈으로
            </Link>
            <Link href={appendUserIdToPath("/chat/new", resolvedUserId)} className="rounded-full border border-[var(--line)] bg-white px-4 py-3 text-sm font-semibold text-[var(--text)]">
              새 상담 시작
            </Link>
          </div>
        </section>
