...
import {
  CloudSqlAdminDashboardPortAdapter,
  CloudSqlAdminUserPortAdapter,
} from "./cloud-sql-admin-dashboard-port";
...
describe("CloudSqlAdminDashboardPortAdapter", () => {
  const adapter = new CloudSqlAdminDashboardPortAdapter();
  ...
  it("hides managed Schift workflows from the runtime workflow list", async () => {
    ...
    expect(dashboard.workflowRules.map((rule) => rule.id)).toEqual(
      expect.not.arrayContaining(["schift-wf-1"]),
    );
  });

  it("hides the legacy monolith workflow from SQL and Schift sources", async () => {
    seedEmptyDashboardQueries();
