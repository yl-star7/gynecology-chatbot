...
export function getPatientNotificationTimeParts(value?: string | null) {
  const normalized = normalizePatientNotificationTimeInput(value ?? "");
  const [hourText, minuteText] = (
    normalized ?? DEFAULT_NOTIFICATION_TIME
  ).split(":");
...
export function buildPatientNotificationTimeFromParts(
  hour: number,
  minute: number,
) {
  return `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`;
