 ...
 *
 * Usage: NODE_TLS_REJECT_UNAUTHORIZED=0 pnpm tsx scripts/sync-pregnancy-to-schift.ts
 */
...
function buildWeekDocument(week: WeekRow): string {
  ...
  if (week.recommended_actions) lines.push(`권장 조치: ${week.recommended_actions}`);
  return lines.join("\n");
}

function buildDayDocument(day: DayRow, checklists: ChecklistRow[], questions: QuestionRow[]): string {
  const lines = [
    `# 임신 ${day.week_number}주 ${day.day_number}일차`,
    "",
  ];
...
async function main() {
  ...
  console.log(`  Weeks: ${weeks.length}, Days: ${days.length}, Checklists: ${checklists.length}, Questions: ${questions.length}`);
  ...
  for (const week of weeks) {
    ...
    knowledgeFiles.push(new File([blob], `week-${week.week_number}-overview.txt`, { type: "text/plain" }));
  }

  for (const day of days) {
    const content = buildDayDocument(day, checklists, questions);
    const blob = new Blob([content], { type: "text/plain" });
    knowledgeFiles.push(new File([blob], `week-${day.week_number}-day-${day.day_number}.txt`, { type: "text/plain" }));
  }

  console.log(`Built ${knowledgeFiles.length} text documents for knowledge bucket`);

  const schift = new Schift({ apiKey: SCHIFT_API_KEY });

  // --- 1) Upload raw docx to pregnancy-raw bucket ---
  const DOCX_PATH = "/Users/jskang/Downloads/임신 주수 별 발달정보(0320_room).docx";
  try {
    ...
    if (fs.existsSync(DOCX_PATH)) {
      ...
      const rawResult = await schift.db.upload(BUCKET_RAW, { files: [rawFile] });
      console.log(`  ✓ Raw bucket: ${rawResult.bucket_name}, uploaded: ${(rawResult.uploaded as unknown[]).length}`);
    } else {
      console.log(`\nSkipping raw docx upload — file not found: ${DOCX_PATH}`);
    }
  } catch (e) {
  ...
  for (let i = 0; i < knowledgeFiles.length; i += BATCH_SIZE) {
    ...
    while (retries > 0) {
      try {
        const result = await schift.db.upload(BUCKET_KNOWLEDGE, { files: batch });
