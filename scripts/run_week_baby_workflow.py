...
WORKFLOW_PATH = PROJECT_ROOT / "output" / "comfy-workflows" / "fetus_compare_object_zimage_t2i.workflow.json"
...
def build_prompt_text(week: dict) -> str:
    ...
    compare_map = {
        ...
        "콜리플라워": "cauliflower",
        "가지": "eggplant",
        "땅콩 호박": "butternut squash",
        "양배추": "cabbage",
        "코코넛": "coconut",
        "샐러리": "celery",
        "파인애플": "pineapple",
        "멜론": "melon",
        "허니듀 멜론": "honeydew melon",
    ...
    compare_object = compare_map.get(compare_object, compare_object)
    return (
        f"Prompt harness: no text, no letters, no label, no caption, no typography. "
        f"A cute educational 3D claymorphism prenatal growth model on a pure white background. "
        f"{week_number}-week prenatal development stage shown as an abstract peach clay prenatal model "
        f"with a rounded head, tiny dot eyes, tiny nose, small mouth, smooth tucked pose, and no explicit anatomy details. "
        f"The prenatal model is placed next to a {compare_object} for size comparison. "
        f"Minimal medical education style, centered composition, seamless pure white studio background, no text."
    )


def build_negative_text() -> str:
    return (
        "text, letters, words, typography, caption, watermark, logo, frame, border text, korean text, english text, "
        "human hand, arm, fingers, realistic anatomy, gore, blood, womb interior, placenta, "
        "multiple objects, duplicate baby, dark background, gray background, gradient backdrop, shadowy scene"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the dedicated fetus comparison workflow through ComfyUI.")
...
def main() -> int:
    ...
    for index, week in enumerate(weeks):
        ...
        workflow["5"]["inputs"]["text"] = build_prompt_text(week)
        workflow["6"]["inputs"]["text"] = build_negative_text()
        workflow["8"]["inputs"]["seed"] = args.seed_base + index
        workflow["10"]["inputs"]["filename_prefix"] = f"week-baby-w{week['weekNumber']:02d}"

        prompt_id = queue_prompt(args.server, workflow)
        output = wait_for_output(args.server, prompt_id, "10")
