...
-- ===== Week 13 =====
UPDATE content.pregnancy_week_data SET baby_summary = '아기의 크기는 레몬만큼, 약 7.4cm / 81g이에요.', mother_summary = '입덧과 피로가 완화되며, 기운이 조금씩 돌아오기 시작해요. 가슴·배에 파란 정맥이 파랗게 드러날 수도 있어요.임신 중에는 혈액이 많이 필요해서 30~50%가량 혈액량이 증가하기 때문이에요.', updated_at = timezone('utc', now()) WHERE week_number = 13;

-- 13주 1일차
UPDATE content.pregnancy_day_contents SET baby_development_payload = '{"items": ["아기의 크기는 레몬만큼, 약 7.4cm / 81g이에요."]}'::jsonb, mother_changes_payload = '{"items": ["입덧과 피로가 완화되며, 기운이 조금씩 돌아오기 시작해요.", "가슴·배에 파란 정맥이 파랗게 드러날 수도 있어요.임신 중에는 혈액이 많이 필요해서 30~50%가량 혈액량이 증가하기 때문이에요.", "체중이 서서히 늘고, 자궁이 골반 밖으로 올라오면서 아랫배가 도드라질 수 있어요."]}'::jsonb, baby_message = '아가는 이제 레몬만큼 커졌어요. 여전히 머리가 더 무겁지만 몸도 점점 커지면서 아주 작은 아기처럼 엄마 배 속에서 크고 있어요.', updated_at = timezone('utc', now()) WHERE week_data_id = (SELECT id FROM content.pregnancy_week_data WHERE week_number = 13) AND day_number = 1;

-- 13주 2일차
UPDATE content.pregnancy_day_contents SET baby_development_payload = '{"items": ["두개골과 긴 뼈가 단단해지고, 치아 구조·손톱·발톱이 형성돼요.", "손목과 발목도 만들어집니다.", "눈은 머리 옆에서 앞으로 이동해 제자리를 찾아가고, 얼굴 윤곽이 뚜렷해지고 있어요."]}'::jsonb, mother_changes_payload = '{"items": ["임신 중기에 들어서면서 조금 편안해지지만,아직 초기의 증상이 바로 사라지지 않을 수 있고,불편한 증상도 나타날 수 있어요.", "코막힘, 속쓰림, 잇몸 출혈 같은 불편감이 생길 수 있어요.이는 혈액량 증가와 호르몬 변화 때문이며 대부분 정상이에요.잇몸에 작은 출혈은 호르몬 영향이지만,통증이 심하거나 염증이 동반되면 검진 때 알려야 해요!", "속쓰림이 심하다면 식사 후 껌을 씹어 위산을 중화시켜보세요!그래도 증상이 나아지지 않는다면 위험하지 않은 속쓰림 약을 추천해드릴게요."]}'::jsonb, baby_message = '아가는 얼굴 윤곽이 두드러지고, 손목과 발목도 생기고 있어요.', updated_at = timezone('utc', now()) WHERE week_data_id = (SELECT id FROM content.pregnancy_week_data WHERE week_number = 13) AND day_number = 2;

-- 13주 3일차
UPDATE content.pregnancy_day_contents SET baby_development_payload = '{"items": ["아기는 양수를 삼키고 소변을 만들어 배출할 수 있어요.", "삼킨 양수는 장에 모여 첫 대변인 ‘태변’을 이룹니다."]}'::jsonb, mother_changes_payload = '{"items": ["입덧이 줄어들며 식욕이 돌아오고, 체중이 늘기 시작해요.", "이제 본격적인 영양 관리가 필요해요 — 아기와 엄마 모두를 위한 시간이에요."]}'::jsonb, baby_message = '아가는 물을 삼키고 배변하는 연습을 해요. 세상에 나가서 잘 먹고 쉴 준비 중이에요.', updated_at = timezone('utc', now()) WHERE week_data_id = (SELECT id FROM content.pregnancy_week_data WHERE week_number = 13) AND day_number = 3;
...
-- ===== Week 19 =====
UPDATE content.pregnancy_week_data SET baby_summary = '오늘 아기는 키 약 24–25cm, 몸무게 약 270–300g, 손에 쥘 수 있는 석류 하나 크기만큼 자랐어요. 손가락과 발가락에는 영원히 변하지 않을 지문과 발자국 무늬가 드디어 완성되었어요. 일란성 쌍둥이도 서로 다른, 오직 자기만의 무늬예요.', mother_summary = '자궁의 끝부분이 배꼽 근처까지 올라와 배가 더 ‘임산부 배’처럼 둥글게 보이기 시작해요. 체형에 따라 어떤 사람은 배가 많이 나온 것 같고, 어떤 사람은 덜 나온 것처럼 보여서 걱정되기도 하지만, 의사가 아기 성장과 체중 증가를 괜찮다고 한다면 배 모양과 크기 차이는 전부 정상 범위예요.', updated_at = timezone('utc', now()) WHERE week_number = 19;

-- 19주 1일차
UPDATE content.pregnancy_day_contents SET baby_development_payload = '{"items": ["오늘 아기는 키 약 24–25cm, 몸무게 약 270–300g, 손에 쥘 수 있는 석류 하나 크기만큼 자랐어요.", "손가락과 발가락에는 영원히 변하지 않을 지문과 발자국 무늬가 드디어 완성되었어요. 일란성 쌍둥이도 서로 다른, 오직 자기만의 무늬예요."]}'::jsonb, mother_changes_payload = '{"items": ["자궁의 끝부분이 배꼽 근처까지 올라와 배가 더 ‘임산부 배’처럼 둥글게 보이기 시작해요.", "체형에 따라 어떤 사람은 배가 많이 나온 것 같고, 어떤 사람은 덜 나온 것처럼 보여서 걱정되기도 하지만, 의사가 아기 성장과 체중 증가를 괜찮다고 한다면 배 모양과 크기 차이는 전부 정상 범위예요."]}'::jsonb, baby_message = '아기는 이제 세상 어디에도 없는 나만의 무늬가 생겼어요. 엄마 품에 안길 날을 기다리며 조용히 준비하고 있어요.', updated_at = timezone('utc', now()) WHERE week_data_id = (SELECT id FROM content.pregnancy_week_data WHERE week_number = 19) AND day_number = 1;
